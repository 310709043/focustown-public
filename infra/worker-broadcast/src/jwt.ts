 ...
 * by the backend's BroadcastTokenService — every change to the claim set
 * must be made in both places (see broadcast_token_service.py).
 *
...
export async function verifyBroadcastToken(
  ...
  const payload = decodeJsonSegment(payloadB64) as Partial<BroadcastClaims>;
  if (typeof payload.exp !== "number" || payload.exp <= nowSeconds) {
    throw new JwtError("expired");
  }
  if (!payload.sub || !payload.cid || !payload.key) {
