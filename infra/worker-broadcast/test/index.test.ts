import { env, createExecutionContext } from "cloudflare:test";
import { describe, expect, it, beforeAll } from "vitest";

import worker from "../src/index";

const SECRET = "test-broadcast-secret-please-rotate-32chars";
const ALLOWED_REFERER = "https://lowbatterytown.com/town";

function base64Url(input: Uint8Array | string): string {
  const bytes =
    typeof input === "string" ? new TextEncoder().encode(input) : input;
  let bin = "";
  for (let i = 0; i < bytes.length; i += 1) bin += String.fromCharCode(bytes[i]);
  return btoa(bin).replace(/=+$/, "").replace(/\+/g, "-").replace(/\//g, "_");
}

async function sign(payload: Record<string, unknown>): Promise<string> {
  const header = base64Url(JSON.stringify({ alg: "HS256", typ: "JWT" }));
  const body = base64Url(JSON.stringify(payload));
  const enc = new TextEncoder();
  const key = await crypto.subtle.importKey(
    "raw",
    enc.encode(SECRET),
    { name: "HMAC", hash: "SHA-256" },
    false,
    ["sign"],
  );
  const sig = new Uint8Array(
    await crypto.subtle.sign("HMAC", key, enc.encode(`${header}.${body}`)),
  );
  return `${header}.${body}.${base64Url(sig)}`;
}

const now = () => Math.floor(Date.now() / 1000);

async function call(path: string, init: RequestInit = {}) {
  const headers = new Headers(init.headers);
  if (!headers.has("referer")) headers.set("referer", ALLOWED_REFERER);
  const req = new Request(`https://broadcast.focustown.app${path}`, {
    ...init,
    headers,
  });
  createExecutionContext();
  return worker.fetch(req, env as any);
}

beforeAll(async () => {
  await (env as any).BROADCAST_BUCKET.put(
    "broadcasts/clip-01.mp4",
    "the-mp4-bytes",
    {
      httpMetadata: { contentType: "video/mp4" },
    },
  );
});

describe("broadcast worker", () => {
  it("rejects request without Referer when allowlist is set", async () => {
    const req = new Request(
      "https://broadcast.focustown.app/clip/clip-01?t=anything",
      { method: "GET" },
    );
    const res = await worker.fetch(req, env as any);
    expect(res.status).toBe(403);
  });

  it("rejects request with disallowed Referer", async () => {
    const t = await sign({
      sub: "u-1",
      cid: "clip-01",
      key: "broadcasts/clip-01.mp4",
      iat: now(),
      exp: now() + 300,
    });
    const res = await call(`/clip/clip-01?t=${t}`, {
      headers: { referer: "https://evil.example/x" },
    });
    expect(res.status).toBe(403);
  });

  it("rejects missing token with 401", async () => {
    const res = await call("/clip/clip-01");
    expect(res.status).toBe(401);
  });

  it("rejects malformed token with 401", async () => {
    const res = await call("/clip/clip-01?t=not.a.jwt");
    expect(res.status).toBe(401);
  });

  it("rejects expired token with 401", async () => {
    const t = await sign({
      sub: "u-1",
      cid: "clip-01",
      key: "broadcasts/clip-01.mp4",
      iat: now() - 3600,
      exp: now() - 60,
    });
    const res = await call(`/clip/clip-01?t=${t}`);
    expect(res.status).toBe(401);
  });

  it("rejects token whose cid does not match the URL", async () => {
    const t = await sign({
      sub: "u-1",
      cid: "clip-different",
      key: "broadcasts/clip-01.mp4",
      iat: now(),
      exp: now() + 300,
    });
    const res = await call(`/clip/clip-01?t=${t}`);
    expect(res.status).toBe(401);
  });

  it("rejects key outside broadcasts/ prefix with 403", async () => {
    const t = await sign({
      sub: "u-1",
      cid: "clip-01",
      key: "secret/admin-only.mp4",
      iat: now(),
      exp: now() + 300,
    });
    const res = await call(`/clip/clip-01?t=${t}`);
    expect(res.status).toBe(403);
  });

  it("returns 404 when the R2 object is missing", async () => {
    const t = await sign({
      sub: "u-1",
      cid: "clip-01",
      key: "broadcasts/does-not-exist.mp4",
      iat: now(),
      exp: now() + 300,
    });
    const res = await call(`/clip/clip-01?t=${t}`);
    expect(res.status).toBe(404);
  });

  it("streams the full object with 200 + cache headers", async () => {
    const t = await sign({
      sub: "u-1",
      cid: "clip-01",
      key: "broadcasts/clip-01.mp4",
      iat: now(),
      exp: now() + 300,
    });
    const res = await call(`/clip/clip-01?t=${t}`);
    expect(res.status).toBe(200);
    expect(res.headers.get("accept-ranges")).toBe("bytes");
    expect(res.headers.get("cache-control")).toContain("private");
    expect(await res.text()).toBe("the-mp4-bytes");
  });

  it("honors Range header with 206 partial content", async () => {
    const t = await sign({
      sub: "u-1",
      cid: "clip-01",
      key: "broadcasts/clip-01.mp4",
      iat: now(),
      exp: now() + 300,
    });
    const res = await call(`/clip/clip-01?t=${t}`, {
      headers: { range: "bytes=0-3", referer: ALLOWED_REFERER },
    });
    expect(res.status).toBe(206);
    expect(res.headers.get("content-range")).toMatch(/^bytes 0-3\//);
    expect(await res.text()).toBe("the-");
  });

  it("CORS preflight echoes allowed origin", async () => {
    const res = await call("/clip/clip-01", {
      method: "OPTIONS",
      headers: {
        origin: "https://lowbatterytown.com",
        referer: ALLOWED_REFERER,
      },
    });
    expect(res.status).toBe(204);
    expect(res.headers.get("access-control-allow-origin")).toBe(
      "https://lowbatterytown.com",
    );
  });
});
