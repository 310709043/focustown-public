...
- **Before release / deploy** — PRs into `main` need green CI before `backend-deploy.yml` / `mcp-deploy.yml` will run on the develop push (see CI/CD section in `AGENTS.md`).
...
- Unit tests use Moto / in-memory backings, **not** LocalStack and **not** real AWS — those are integration / E2E tiers (see the three-tier table in `AGENTS.md`).
...
`tests/integration/` and `tests/e2e/` are out of scope here — they have separate cost and infrastructure (see the Local environment section in `AGENTS.md`).
