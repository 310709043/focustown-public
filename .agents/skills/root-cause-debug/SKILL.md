...
3. **Auto-apply IFF evidence is sufficient** — when Phase 5 ROOT CAUSE is fully cited (every sentence references a file:line or CloudWatch line), the skill applies the Phase 6 patch via Edit and commits it as ONE evidence-grounded fix commit on the current branch (per `.Codex/rules/git-workflow.md` — current branch only, never `main`). When Phase 5 emits `INSUFFICIENT EVIDENCE`, the skill stops there — never patches on speculation. Re-deploying the affected lambda + re-running `/workflow-test` to confirm Phase 7's expectations is the user's call: the commit makes the change reviewable; deploy is a separate, user-initiated action.
...
The patch must be the **minimal** change that addresses the cited cause. No drive-by improvements, no logging additions, no defensive type checks unless the cited cause is "missing type guard". Per `.Codex/rules/coding-style.md` rule "Don't add error handling… for scenarios that can't happen".
