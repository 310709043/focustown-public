...
- **部署**：AWS Lightsail Container Service + Managed Postgres（prod + dev 雙環境 ~$64/mo），ECR push + GitHub Actions OIDC — 完整步驟見 [infra/lightsail/bootstrap.md](./infra/lightsail/bootstrap.md)
...
3. 想驗證 S3 串接行為（presigned URL + 302 redirect + Range），加上 `docker-compose.s3.yml` override：
   ...
   驗收：MinIO console 內 `lowbatterytown-dev` bucket 應有 `tracks/<uuid>.mp3` × 5；前端 `/town` 的 PersonalRadio 播放時 DevTools Network 看到 `/api/v1/tracks/<id>/stream` 回 302 → `localhost:9000/...?X-Amz-Signature=...`。
...
# 進 Postgres
docker compose exec postgres psql -U lowbatterytown -d lowbatterytown
```
